...
while True:
    try:
        time = int(input("How many hours ago did you start drinking? "))
    except ValueError:
        print("You entered the time wrong incorrectly, try again")
    else:
        break

#makes sure you enter the standard drinkes a float
while True:
    try:
...
#asks what kind license holder they are
while x == 1:
...
if license == "L" or license == "P":
    if BAC >= 0.05:
        print("As a learner with a blood alchohol above 0.05 of", BAC, "your license would be cancelled and an interlock device required")
    else:
